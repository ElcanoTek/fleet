...
export function PromptLibrary({
  ...
  useDialogA11y(open, modalRef, () => setOpen(false), {
  ...
  const load = async () => {
    ...
    try {
      ...
      setItems(next);
      setSelectedID((id) =>
        id && next.some((p) => p.id === id) ? id : (next[0]?.id ?? null),
      );
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Could not load prompt library",
      );
    } finally {
      setLoading(false);
    }
  ...
  const save = async () => {
    ...
    try {
      if (selected && !selected.read_only)
        await orchestratorApi.updatePrompt(selected.id, draft);
  ...
  return (
    <>
      <button
        ...
        data-tip-top={compact ? "Prompt library: reuse prompts" : undefined}
        className={
          compact
      ...
      >
        ...
        {!compact ? (
          <span style={{ marginLeft: 6 }}>Prompt library</span>
        ) : null}
      </button>
      ...
      {open
        ? createPortal(
            ...
            >
              <div
                ref={modalRef}
                className="flex h-[100dvh] w-full flex-col overflow-hidden border border-[var(--color-border-strong)] bg-[var(--color-surface-1)] shadow-2xl sm:h-[min(46rem,92vh)] sm:w-[min(68rem,96vw)] sm:rounded-xl"
              >
                ...
                {error ? (
                  <div className="mx-4 mt-3 rounded-md bg-[color-mix(in_srgb,var(--color-danger)_12%,transparent)] px-3 py-2 text-sm text-[var(--color-danger)]">
                    {error}
                  </div>
                ) : null}
                    ...
                    {editing ? (
                      ...
                      <div className="flex min-h-full flex-col">
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          <div>
                            <h3 className="m-0 text-lg font-semibold text-[var(--color-text-primary)]">
                              {selected.name}
                            </h3>
                          ...
                          <div className="flex gap-2">
                            {!selected.read_only && selected.owned_by_caller ? (
                              <button
                                type="button"
                                className="btn btn-secondary btn-small"
                                onClick={beginEdit}
