...
import { DEFAULT_MODEL, DEFAULT_MODEL_LABEL, setModelTiers, _resetModelTiersForTests } from "@/app/lib/modelAliases";
import {
  compactModelLabel,
  filterModels,
  loadModels,
...
describe("loadModels (fetch + fallback)", () => {
  ...
  it("joins catalog prices onto the hand-written seed entries", async () => {
    ...
    const models = await loadModels();
    const seeded = models.find((m) => m.id === DEFAULT_MODEL);
    ...
    expect(models.filter((m) => m.id === DEFAULT_MODEL)).toHaveLength(1);
  });
