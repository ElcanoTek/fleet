...
| **Recipients** | The email addresses that receive each run's result. They belong to the task rather than the prompt, so the same library prompt can serve different audiences, and they survive an edit — including re-inserting a different prompt from the library. |
