...
| **Advanced** | Further settings, including the model the task runs on and an option for a recurring task to carry a short summary of its previous run into the next one. By default a new task is pre-filled with **GPT-5.6 Luna Pro** as its model and **DeepSeek V4.1 Flash** as its fallback, and it keeps both unless you change them here, even if you never open Advanced; your admin can change those defaults. The model in particular is worth choosing deliberately: match it to the demands of the job rather than leaving it to chance. |
