...
const browserbaseLiveViewDescription = "Turn a Browserbase session id into a live-view URL a human can open to watch and control that hosted browser. " +
	...
	"Call the connector's `_start` tool first and pass the session id it returns. Note the connector's tools are named after the connection the user created (usually `mcp_browserbase_start`, but the prefix is whatever they named it), and the `## MCP Tools (live registry)` section of your system prompt lists exactly what is registered this turn — by name, or, when the roster is large, as connectors reachable through tool_search/tool_call.\n\n" +
