...
func cmdTask(argv []string) int {
	...
	case "run":
		...
		// `fleet` binary BEFORE admincli (see cmd/fleet classifyInvocation), so
		// this case is not reached on any supported path. It stays as a plain
		// answer rather than the generic unknown-subcommand error (#722) for a
		// caller that reaches admincli.Run directly — which is what the retired
		// fleet shim used to do (ADR-0060), and what an embedder could do
		// again.
		return errf(1, "`task run` needs the full agent runtime and is dispatched by the fleet binary itself; run `fleet task run <task.yaml>`")
	default:
