...
func cmdDoctor(argv []string) int {
	...
	// Cancel with SIGTERM, not the default SIGKILL, and never force a kill
	// after it (no WaitDelay): while doctor has fleet stopped for a podman
	// store reset (scripts/lib/podman-migrate.sh, migrate_live_service) its
	// trap must be able to start fleet again, and bash cannot trap SIGKILL.
	// bash runs that trap only after a foreground migrate returns, and the
	// start itself may take the unit's TimeoutStartSec, so any deadline here
	// could cut the restore short. Outside that window bash exits on SIGTERM
	// at once, so nothing hangs.
	cmd.Cancel = func() error { return cmd.Process.Signal(syscall.SIGTERM) }
	// After the first signal, stop intercepting: a second Ctrl-C exits
	// `fleet doctor` itself, while the script carries on restoring fleet.
