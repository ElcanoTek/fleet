...
func schedAPIKeyCreate(argv []string) int {
	...
	if t := strings.TrimSpace(*keyType); t != "" {
		...
		key, raw, err := mgr.CreateTypedKey(name, kt, slugs, *rateLimit, nil, "created via fleet")
		if err != nil {
			return errf(5, "%v", err)
		}
		ks.fixOwnership()
	...
	roleVal := *role
	key, raw, err := mgr.CreateKey(name, nil, &roleVal, *rateLimit, nil, "created via fleet")
	if err != nil {
		return errf(5, "%v", err)
	}
	ks.fixOwnership()
