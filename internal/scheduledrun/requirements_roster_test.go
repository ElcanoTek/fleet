...
func TestExecutionRequirementsRoster(t *testing.T) {
	for _, tc := range []struct {
		...
		req, err := parseExecutionRequirements(tc.prompt)
		if got := req.rosterNarrowing(); got != tc.want || (err != nil) != tc.wantErr {
			t.Fatalf("roster of %.60q = %q, %v; want %q, err=%t", tc.prompt, got, err, tc.want, tc.wantErr)
		}
...
func TestRunWorkerRejectsAnUnknownRosterBeforeModelSetup(t *testing.T) {
	task := &models.Task{Prompt: models.ExecutionRequirementsMarker + "\n" + `{"roster":"all_tools"}`}
...
func TestNarrowedAllowlist(t *testing.T) {
	req := &executionRequirements{ExecutionRequirements: models.ExecutionRequirements{Tools: []string{
	...
	got2 := (&executionRequirements{ExecutionRequirements: models.ExecutionRequirements{Tools: []string{"mcp_foo_search"}}}).narrowedAllowlist(prefixed, nil)
	...
	withCompletion := &executionRequirements{ExecutionRequirements: models.ExecutionRequirements{
	...
	bare := (&executionRequirements{ExecutionRequirements: models.ExecutionRequirements{Tools: []string{"get_page_data"}}}).narrowedAllowlist(pagesCatalog(), nil)
	...
	nativeOnly := (&executionRequirements{ExecutionRequirements: models.ExecutionRequirements{Tools: []string{"run_python"}}}).narrowedAllowlist(pagesCatalog(), nil)
