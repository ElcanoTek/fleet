...
_NAMED = {
    "transparent": (0, 0, 0, 0.0), "none": (0, 0, 0, 0.0),
    "black": (0, 0, 0, 1.0), "white": (1, 1, 1, 1.0),
    "red": (1, 0, 0, 1.0), "green": (0, 0.502, 0, 1.0),
    "blue": (0, 0, 1, 1.0), "gray": (0.502, 0.502, 0.502, 1.0),
    "grey": (0.502, 0.502, 0.502, 1.0), "silver": (0.753, 0.753, 0.753, 1.0),
    "navy": (0, 0, 0.502, 1.0), "teal": (0, 0.502, 0.502, 1.0),
    "orange": (1, 0.647, 0, 1.0), "yellow": (1, 1, 0, 1.0),
    "purple": (0.502, 0, 0.502, 1.0), "inherit": None, "currentcolor": None,
}
...
WIDTHS = {name: [int(n) for n in src.split()]
          for name, src in _WINANSI_WIDTHS_SRC.items()}
...
_SERIF_HINTS = ("serif", "georgia", "times", "garamond", "fraunces", "playfair",
                "merriweather", "cambria", "book", "charter", "spectral", "lora",
                "source serif", "pt serif", "noto serif", "ibm plex serif",
                "instrument serif", "newsreader", "literata", "bitter")
_MONO_HINTS = ("mono", "courier", "consolas", "menlo", "sf mono", "jetbrains",
               "fira code", "source code", "ibm plex mono", "roboto mono")
...
class TextEncoder:
    ...
    def warning(self):
        ...
        return ("%d character(s) have no glyph in the PDF core fonts and were "
                "written as '?' (%s). Text outside Western European scripts "
                "needs the deck's own Export PDF (print) button."
                % (self.dropped, shown))
...
def parse_inline(html):
    ...
    def push(chunk):
        ...
        if not chunk:
            return
        runs.append(Run(unescape(chunk), bold_depth > 0, italic_depth > 0,
                        mono_depth > 0, pending_break))
        pending_break = False
    ...
    push(text[pos:])
    if pending_break:
        runs.append(Run("", bold_depth > 0, italic_depth > 0, mono_depth > 0,
                        True))
    return runs
...
def layout_text(encoder, runs, box_width, family, size, weight,
                letter_spacing=0.0, wrap=True):
        ...
        for token in tokens:
            ...
            if not wrap or blank or line.width + width <= box_width + 0.01:
                if wrap and blank and width > box_width + 0.01:
                    for part in _break_word(encoder, token, face, run_size,
                                            box_width, letter_spacing):
                        if lines[-1].pieces:
                            lines.append(Line())
                        lines[-1].add(part)
                    continue
...
class PdfWriter:
    ...
    def put(self, number, body):
        self._objects[number] = body if isinstance(body, bytes) \
            else body.encode("latin-1")
    ...
    def serialize(self, root, info):
        ...
        out += ("trailer\n<< /Size %d /Root %d 0 R /Info %d 0 R >>\n"
                "startxref\n%d\n%%%%EOF\n"
                % (len(self._objects), root, info, start)).encode("latin-1")
        return bytes(out)
...
class Image:
    ...
    __slots__ = ("width", "height", "data", "filter", "colorspace", "bpc",
                 "smask", "palette")

    def __init__(self, width, height, data, filt, colorspace, bpc=8,
                 smask=None, palette=None):
        self.width = width
        self.height = height
        self.data = data
        self.filter = filt
...
def _png_decode(data):
    ...
    for row in range(height):
        ...
        if filt == 1:
            ...
            for i in range(stride):
                ...
                da, db, dc = (abs(peak - left), abs(peak - up),
                              abs(peak - upper_left))
                nearest = left if (da <= db and da <= dc) else \
                    (up if db <= dc else upper_left)
    ...
    if alpha is not None and min(alpha) < 255:
        smask = Image(width, height, zlib.compress(bytes(alpha), 9),
                      "/FlateDecode", "/DeviceGray")
    del ncomp
    return Image(width, height, zlib.compress(bytes(colour), 9),
                 "/FlateDecode", space, 8, smask, palette)
...
class Resources:
    ...
    def image(self, image, cache_key=None):
        ...
        entries = ["/Type /XObject", "/Subtype /Image",
                   "/Width %d" % image.width, "/Height %d" % image.height,
        ...
        if image.palette is not None:
            ...
            entries.append("/ColorSpace [/Indexed /DeviceRGB %d %d 0 R]"
        ...
        number = self.writer.add(
            head.encode("latin-1") + image.data + b"\nendstream")
    ...
    def shading(self, coords, stops):
        ...
        stops = sorted(((max(0.0, min(1.0, at)), rgb) for at, rgb in stops),
        ...
        for index in range(len(stops) - 1):
            ...
            functions.append(self.writer.add(
        ...
        if len(functions) == 1:
            ...
            combined = self.writer.add(
                ...
                % (" ".join("%d 0 R" % f for f in functions),
                   " ".join(num(b) for b in bounds), " ".join(encode)))
        number = self.writer.add(
    ...
    def dictionary(self):
        def group(items):
            return " ".join("%s %d 0 R" % (name, number)
                            for name, number in sorted(items,
                                                       key=lambda pair: pair[0]))
...
class Canvas:
    ...
    def rect_path(self, x, y, w, h, radius=0):
        ...
        self.op("%s %s %s %s %s %s c"
                % (num(right - radius + k), num(y), num(right),
                   num(y + radius - k), num(right), num(y + radius)))
        self.op("%s %s l" % (num(right), num(bottom - radius)))
        self.op("%s %s %s %s %s %s c"
                % (num(right), num(bottom - radius + k),
                   num(right - radius + k), num(bottom),
                   num(right - radius), num(bottom)))
        self.op("%s %s l" % (num(x + radius), num(bottom)))
        self.op("%s %s %s %s %s %s c"
                % (num(x + radius - k), num(bottom), num(x),
                   num(bottom - radius + k), num(x), num(bottom - radius)))
        self.op("%s %s l" % (num(x), num(y + radius)))
        self.op("%s %s %s %s %s %s c"
                % (num(x), num(y + radius - k), num(x + radius - k), num(y),
                   num(x + radius), num(y)))
        self.op("h")
    ...
    def ellipse_path(self, cx, cy, rx, ry):
        ...
        self.op("%s %s %s %s %s %s c" % (num(cx + rx), num(cy + ky),
                                         num(cx + kx), num(cy + ry),
                                         num(cx), num(cy + ry)))
        self.op("%s %s %s %s %s %s c" % (num(cx - kx), num(cy + ry),
                                         num(cx - rx), num(cy + ky),
                                         num(cx - rx), num(cy)))
        self.op("%s %s %s %s %s %s c" % (num(cx - rx), num(cy - ky),
                                         num(cx - kx), num(cy - ry),
    ...
    def paint(self, fill=None, stroke=None, width=1.0, dash=None, cap=None,
              even_odd=False):
        """Close out the current path with the right painting operator."""
        self.apply_alpha(fill[3] if is_visible(fill) else 1.0,
...
def visible_slides(doc):
    ...
    return [s for s in slides
            if isinstance(s, dict) and not s.get("stateOf")
...
class Renderer:
    def __init__(self, doc):
        ...
        self.assets = doc.get("assets") if isinstance(doc.get("assets"),
                                                     dict) else {}
    ...
    def image_for(self, src):
        ...
        if not resolved.startswith("data:"):
            self.warn("image sources that are not embedded (%s...) were left "
                      "out: a PDF has no network, so only data: URIs and "
                      "doc.assets entries can be drawn."
                      % resolved[:32], "remote-image")
            return None
        image, reason = decode_image(resolved)
        if image is None:
            self.warn("an embedded image was skipped because %s. The built-in "
                      "export draws PNG (non-interlaced) and JPEG; re-embed it "
                      "in one of those, or use the deck's own PDF export."
                      % reason, "image-%s" % reason)
            return None
    ...
    def render(self):
        ...
        if not slides:
            raise PdfError("the document has no printable slides (every slide "
        ...
        for number_, content in zip(pages, contents):
            self.writer.put(number_,
                            "<< /Type /Page /Parent %d 0 R /MediaBox "
                            "[0 0 %s %s] /Resources %d 0 R /Contents %d 0 R >>"
                            % (tree, num(self.page_width),
                               num(self.page_height), resources, content))
        self.writer.put(tree,
        ...
        info = self.writer.add(
            ...
            "/CreationDate (D:%s) >>"
            % (pdf_string(title).decode("latin-1"),
    ...
    def render_slide(self, canvas, slide, fields):
        ...
        for element in elements if isinstance(elements, list) else []:
            ...
            try:
                ...
                self.warn("element %r (%s) could not be drawn: %s"
    ...
    def render_element(self, canvas, element, fields):
        ...
        opacity = 1.0 if not isinstance(opacity, (int, float)) \
        ...
        if element.get("blur") or element.get("shadow") or \
                element.get("blend") or element.get("backdropFilter"):
            self.warn("blur, drop shadow, blend and backdrop-filter effects "
                      "are not reproduced by the built-in export; the shapes "
                      "and text are drawn without them.", "filters")
        box = (x, y, w, h)
        if kind == "text":
            ...
            self.warn("an `svg` element was skipped: the built-in export has "
                      "no SVG renderer. Compose the artwork from shape "
                      "elements, or use the deck's own PDF export.", "svg")
        else:
            self.warn("unknown element type %r was skipped." % kind,
                      "type-%s" % kind)
        canvas.restore()
    ...
    def render_text(self, canvas, element, box, fields):
        ...
        line_height = float(line_height) if isinstance(
            line_height, (int, float)) and line_height > 0 else 1.2
        family = family_of(element.get("fontFamily") or self.theme_font)
        letter_spacing = number(element.get("letterSpacing"))
        lines = layout_text(self.encoder, runs, w, family, size,
                            element.get("fontWeight"), letter_spacing)
        ...
        if stops:
            ...
            canvas.save()
            mode = 7
        elif isinstance(stroke, dict) and number(stroke.get("width")) > 0 \
        ...
        for index, line in enumerate(lines):
            ...
            for piece in line.pieces:
                if not piece.text.strip():
                    continue
                canvas.show_text(start + piece.x, baseline, piece.text,
                                 piece.face, piece.size, letter_spacing)
        canvas.end_text()
        if stops:
            ...
            self.paint_gradient(canvas, gradient, stops,
    ...
    def gradient_coords(self, gradient, box):
        ...
        return (x + (0.5 - dx) * w, y + (0.5 - dy) * h,
                x + (0.5 + dx) * w, y + (0.5 + dy) * h)
    ...
    while index < len(tokens):
        ...
        try:
            if lower == "m":
                ...
                if relative:
                    x1, y1, x2, y2, px, py = (cx + x1, cy + y1, cx + x2,
                                              cy + y2, cx + px, cy + py)
                ...
                if relative:
                    px, py = cx + px, cy + py
                out.extend(_arc_to_beziers(cx, cy, rx, ry, rot, large, sweep,
                                          px, py))
                cx, cy = px, py
            else:
                break
...
def _arc_to_beziers(x1, y1, rx, ry, rotation, large, sweep, x2, y2):
    ...
    factor = 0.0 if denom == 0 else max(
        0.0, (rx * rx * ry * ry - denom) / denom)
    ...
    for _ in range(segments):
        ...
        out.append(("c", (px1 + k * dx1, py1 + k * dy1),
                    (px2 - k * dx2, py2 - k * dy2), (px2, py2)))
        theta += step
    return out
...
def _dash_pattern(element, width):
    ...
    if style not in (None, "solid") and isinstance(dash, (int, float)) \
            and dash > 0:
...
class ShapeMixin:
    def render_shape(self, canvas, element, box):
        ...
        if shape == "rect":
            canvas.rect_path(inset, inset, max(w - stroke_width, 0),
                             max(h - stroke_width, 0),
                             number(element.get("radius")))
        elif shape == "ellipse":
            canvas.ellipse_path(w / 2.0, h / 2.0, max(w / 2.0 - inset, 0),
                                max(h / 2.0 - inset, 0))
        elif shape == "triangle":
            canvas.polygon_path([(w / 2.0, inset), (w - inset, h - inset),
                                 (inset, h - inset)])
        elif shape == "arrow":
            shaft = h * 0.44
            head = min(w * 0.38, h)
            top = (h - shaft) / 2.0
            canvas.polygon_path([(0, top), (w - head, top), (w - head, 0),
                                 (w, h / 2.0), (w - head, h),
                                 (w - head, top + shaft), (0, top + shaft)])
        elif shape == "path":
            self.emit_path(canvas, element, w, h)
        else:
            self.warn("unknown shape %r was skipped." % shape,
                      "shape-%s" % shape)
            canvas.restore()
            return
        ...
        if stops:
            canvas.save()
            canvas.clip()
            self.paint_gradient(canvas, element.get("fillGradient"), stops,
                                self.gradient_coords(element.get("fillGradient"),
                                                     (0, 0, w, h)),
                                (0, 0, w, h))
            canvas.restore()
            if is_visible(stroke) and stroke_width > 0:
                ...
                self.render_shape_outline(canvas, element, box, stroke,
                                          stroke_width, dash, cap)
        else:
            canvas.paint(fill=fill, stroke=stroke, width=stroke_width,
                         dash=dash, cap=cap)
        canvas.restore()

    def render_shape_outline(self, canvas, element, box, stroke, width, dash,
                             cap):
        _, _, w, h = box
        inset = width / 2.0
        shape = element.get("shape") or "rect"
        if shape == "rect":
            canvas.rect_path(inset, inset, max(w - width, 0), max(h - width, 0),
                             number(element.get("radius")))
        elif shape == "ellipse":
            canvas.ellipse_path(w / 2.0, h / 2.0, max(w / 2.0 - inset, 0),
                                max(h / 2.0 - inset, 0))
    ...
    def emit_path(self, canvas, element, w, h):
        ...
        if isinstance(pbox, list) and len(pbox) == 4 and \
                all(isinstance(v, (int, float)) for v in pbox):
    ...
    def render_line(self, canvas, element, box):
        ...
        canvas.paint(stroke=color, width=width, dash=dash,
                     cap=0 if element.get("strokeStyle") == "dashed" else 1)
        for kind, at_start in ((element.get("lineStart"), True),
                               (element.get("lineEnd"), False)):
            if kind and kind != "none":
                self.render_tip(canvas, kind, color, width,
                                x + (start if at_start else w - end), mid,
                                at_start)
        canvas.restore()
    ...
    def render_tip(self, canvas, kind, color, width, at_x, at_y, reversed_):
        ...
        if kind == "arrow":
            ...
            canvas.rect_path(at_x - 0.8 * unit, at_y - 3.6 * unit,
                             1.6 * unit, 7.2 * unit)
    ...
    def render_image(self, canvas, element, box):
        ...
        if fit in ("cover", "contain") and natural_w and natural_h:
            scale = max(w / natural_w, h / natural_h) if fit == "cover" \
                else min(w / natural_w, h / natural_h)
            draw_w, draw_h = natural_w * scale, natural_h * scale
            canvas.draw_image(name, x + (w - draw_w) / 2.0,
                              y + (h - draw_h) / 2.0, draw_w, draw_h)
        else:
    ...
    def render_media(self, canvas, element, box):
        ...
        if got:
            ...
            if kind == "video":
                canvas.polygon_path([(cx - size * 0.4, cy - size * 0.55),
                                     (cx + size * 0.6, cy),
                                     (cx - size * 0.4, cy + size * 0.55)])
                canvas.paint(fill=glyph)
            else:
                canvas.ellipse_path(cx - size * 0.25, cy + size * 0.35,
                                    size * 0.28, size * 0.22)
                canvas.paint(fill=glyph)
                canvas.rect_path(cx, cy - size * 0.6, size * 0.12, size * 0.95)
                canvas.paint(fill=glyph)
            self.warn("video and audio elements are drawn as a poster block — "
                      "a PDF cannot play media.", "media")
        canvas.restore()
...
class TableMixin:
    ...
    def render_table(self, canvas, element, box):
        ...
        if not isinstance(columns, list) or not isinstance(rows, list) \
                or not columns or not rows:
            return
        style = element.get("style") if isinstance(element.get("style"),
                                                  dict) else {}
        weights = [max(0.0, number(c.get("w"))) if isinstance(c, dict) else 0.0
                   for c in columns]
        ...
        for row_index, row in enumerate(rows):
            ...
            for column_index in range(len(widths)):
                ...
                inner = max(widths[column_index] - pad_x * 2 - border_width * 2,
                            1.0)
                lines = layout_text(self.encoder,
                                    parse_inline(str(cell.get("html") or "")),
                                    inner, family, font_size,
                                    700 if bold else 400)
                laid.append((cell, lines, bold))
                tallest = max(tallest, len(lines) * font_size * line_height
                              + pad_y * 2)
            grid.append(laid)
        ...
        for row_index, laid in enumerate(grid):
            ...
            for column_index, (cell, lines, bold) in enumerate(laid):
                ...
                for line_index, line in enumerate(lines):
                    baseline = (cell_top + line_index * step + half_leading
                                + ascent * font_size)
                    start = inner_left + (inner_width - line.width) * align
                    for piece in line.pieces:
                        if piece.text.strip():
                            canvas.show_text(start + piece.x, baseline,
                                             piece.text, piece.face,
                                             piece.size)
                left += width
...
CHART_COLORS = ["#5470c6", "#91cc75", "#fac858", "#ee6666",
...
def nice_ticks(low, high, count=5):
    ...
    step = power * (10 if ratio >= 7.5 else 5 if ratio >= 3.5
                    else 2 if ratio >= 1.5 else 1)
...
def _step_ticks(low, high, count):
    def magnitude(value):
        ...
        pick = 1 if mantissa <= 1 else 2 if mantissa <= 2 else \
            5 if mantissa <= 5 else 10
...
def axis_scale(values, axis, tick_count=None):
    ...
    if tick_count:
        ticks = _even_ticks(bottom, top, tick_count) if pinned \
            else _step_ticks(bottom, top, tick_count - 1)
    else:
        ticks = _even_ticks(bottom, top, 6) if pinned \
            else nice_ticks(bottom, top)
    formatter = axis.get("formatter")
    labels = [formatter.replace("{value}", format_number(t)) if formatter
              else format_number(t) for t in ticks]
...
def normalize_chart(option, width, height):
    ...
    if legend_option:
        ...
        text_style = legend_option.get("textStyle") \
            if isinstance(legend_option.get("textStyle"), dict) else {}
        legend = {
            ...
            if isinstance(legend_option.get("top"), (int, float)) else None,
    ...
    x_label = x_axis.get("axisLabel") \
        if isinstance(x_axis.get("axisLabel"), dict) else {}
    ...
    for axis in y_option[:2]:
        ...
        label = axis.get("axisLabel") \
            if isinstance(axis.get("axisLabel"), dict) else {}
        y_axes.append({
            "name": axis.get("name") if isinstance(axis.get("name"), str)
            else None,
            "min": axis.get("min") if isinstance(axis.get("min"),
                                                 (int, float)) else None,
            "max": axis.get("max") if isinstance(axis.get("max"),
                                                 (int, float)) else None,
            "formatter": label.get("formatter")
            if isinstance(label.get("formatter"), str) else None,
            "label": {
    ...
    bottom_band = legend["bottom"] + band \
        if legend and legend["top"] is None else 0
    overflow = max(0.0, band - 12)
    grid_bottom = opt_num(grid.get("bottom"), 44) if "bottom" in grid else \
        44 + (overflow if legend and legend["top"] is None else 0)
    grid_top = opt_num(grid.get("top"), 24) if "top" in grid else \
        24 + (overflow if legend and legend["top"] is not None else 0)
    if is_pie:
        plot = {"x": 0.0, "y": top_band, "w": width,
                "h": max(0.0, height - top_band - bottom_band)}
    else:
        left = opt_num(grid.get("left"), 48)
        right = opt_num(grid.get("right"), 56 if dual else 16)
        plot = {"x": left, "y": grid_top, "w": width - left - right,
                "h": max(0.0, height - grid_top - grid_bottom)}
    colors = option.get("color")
    colors = [c for c in colors if isinstance(c, str)] \
        if isinstance(colors, list) and colors else CHART_COLORS
    text_style = option.get("textStyle") \
        if isinstance(option.get("textStyle"), dict) else {}
    axis_line = x_axis.get("axisLine") \
        if isinstance(x_axis.get("axisLine"), dict) else {}
    axis_line_style = axis_line.get("lineStyle") \
        if isinstance(axis_line.get("lineStyle"), dict) else {}
    first_y = y_option[0] if isinstance(y_option[0], dict) else {}
    split = first_y.get("splitLine") \
        if isinstance(first_y.get("splitLine"), dict) else {}
    split_style = split.get("lineStyle") \
        if isinstance(split.get("lineStyle"), dict) else {}
    pie = next((s for s in series if s.get("type") == "pie"), None)
    pie_label = pie.get("label") if isinstance(pie, dict) and \
        isinstance(pie.get("label"), dict) else {}
    categories = x_axis.get("data")
    return {
        "w": width, "h": height,
        "font": family_of(text_style.get("fontFamily") or "sans-serif"),
        "colors": colors, "series": series, "isPie": is_pie,
        "categories": [str(c) for c in categories]
        if isinstance(categories, list) else [],
        "grid": plot, "legend": legend,
        "xAxisLabel": {
...
class ChartMixin:
    def render_chart(self, canvas, element, box):
        ...
        if not chart["series"]:
            ...
            self.warn("a chart element has no series; only its axes were "
                      "drawn (the app does the same).", "chart-empty")
        canvas.save()
        canvas.translate(x, y)
        if chart["isPie"]:
        ...
        if chart["legend"] and any(s.get("name") or s.get("type") == "pie"
                                  for s in chart["series"]):
            self.draw_legend(canvas, chart)
        canvas.restore()

    # -- helpers -----------------------------------------------------------
    def chart_text(self, canvas, chart, cx, baseline, text, color, size,
        ...
        if anchor == "middle":
            ...
            start = cx - width
        rgba = color if isinstance(color, tuple) \
    ...
    def series_color(self, chart, series, index):
        item_style = series.get("itemStyle") \
            if isinstance(series.get("itemStyle"), dict) else {}
        line_style = series.get("lineStyle") \
            if isinstance(series.get("lineStyle"), dict) else {}
    ...
    def draw_cartesian(self, canvas, chart):
        ...
        def axis_index(series):
            return min(axis_count - 1,
                       max(0, int(round(opt_num(series.get("yAxisIndex"), 0)))))
        ...
        for series in points:
            for entry in series.get("data") or []:
                px, py = (entry[0], entry[1]) if isinstance(entry, list) \
                    and len(entry) >= 2 else (0, 0)
                low = min(low, px)
                high = max(high, px)
                buckets[0].append(opt_num(py, 0))
        primary = axis_scale(buckets[0], chart["yAxes"][0])
        ticks = len(primary["labels"])
        secondary = axis_scale(buckets[1], chart["yAxes"][1], ticks) \
            if axis_count > 1 else None
        ...
        for index in range(ticks):
            ...
            canvas.paint(stroke=chart["splitLine"]["color"],
                         width=chart["splitLine"]["width"])
            label = chart["yAxes"][0]["label"]
            self.chart_text(canvas, chart, plot["x"] - 8,
                            line_y + label["size"] * 0.35,
                            primary["labels"][index],
                            parse_color(label["color"], (0.42, 0.45, 0.5, 1)),
                            label["size"], "end", label["weight"])
            if secondary:
                label2 = chart["yAxes"][1]["label"]
                self.chart_text(canvas, chart, plot["x"] + plot["w"] + 8,
                                line_y + label2["size"] * 0.35,
                                secondary["labels"][index],
                                parse_color(label2["color"],
                                            (0.42, 0.45, 0.5, 1)),
                                label2["size"], "start", label2["weight"])
        for axis, anchor, at_x in ((0, "end", plot["x"] - 8),
                                  (1, "start", plot["x"] + plot["w"] + 8)):
            if axis < axis_count and chart["yAxes"][axis].get("name") \
                    and (axis == 0 or secondary):
                self.chart_text(canvas, chart, at_x, plot["y"] - 9,
                                chart["yAxes"][axis]["name"],
                                parse_color(chart["xAxisLabel"]["color"],
                                            (0.42, 0.45, 0.5, 1)), 11, anchor)

        baseline = zero_y(0)
        canvas.line_path(plot["x"], baseline, plot["x"] + plot["w"], baseline)
        canvas.paint(stroke=chart["axisLine"]["color"],
                     width=chart["axisLine"]["width"])

        # A scatter with no categories gets a numeric x axis, like the app.
        if points and not categories:
            span_ticks = nice_ticks(0 if low == float("inf") else min(0, low),
            ...
            for tick in span_ticks:
                self.chart_text(canvas, chart, point_x(tick),
                                plot["y"] + plot["h"]
                                + chart["xAxisLabel"]["size"] + 6,
                                format_number(tick),
                                parse_color(chart["xAxisLabel"]["color"],
                                            (0.42, 0.45, 0.5, 1)),
                                chart["xAxisLabel"]["size"], "middle",
                                chart["xAxisLabel"]["weight"])
            for index, series in enumerate(points):
                color = self.series_color(chart, series,
                                          chart["series"].index(series))
                radius = opt_num(series.get("symbolSize"), 10) / 2.0
                for entry in series.get("data") or []:
                    px, py = (entry[0], entry[1]) \
                        if isinstance(entry, list) and len(entry) >= 2 else (0, 0)
                    at_x = point_x(px)
                    if at_x < plot["x"] - radius or \
                            at_x > plot["x"] + plot["w"] + radius:
                        continue
                    canvas.save()
                    canvas.alpha(0.85)
                    canvas.ellipse_path(at_x, value_y(opt_num(py, 0)), radius,
                                        radius)
                    canvas.paint(fill=color)
                    canvas.restore()
                del index
        ...
        for index, category in enumerate(categories):
            if index % stride:
                continue
            self.chart_text(canvas, chart, plot["x"] + band * (index + 0.5),
                            plot["y"] + plot["h"]
                            + chart["xAxisLabel"]["size"] + 6, category,
                            parse_color(chart["xAxisLabel"]["color"],
                                        (0.42, 0.45, 0.5, 1)),
                            chart["xAxisLabel"]["size"], "middle",
                            chart["xAxisLabel"]["weight"])
        ...
        if bars:
            ...
            for order, series in enumerate(bars):
                ...
                corner = opt_num(corner[0], 0) if isinstance(corner, list) \
                    else opt_num(corner, 0)
                data = series.get("data")
                data = data if isinstance(data, list) else []
                for index, raw in enumerate(data[:count]):
                    value = opt_num(raw, 0)
                    left = plot["x"] + band * index + (band - group) / 2.0 \
                        + slot * order
                    top = value_y(value, axis)
                    height = abs(base - top)
                    canvas.rect_path(left + 1, top if top <= base else base,
                                     max(1.0, slot - 2), max(0.0, height),
                                     min(corner, slot / 2.0))
                    canvas.paint(fill=color)
        ...
        for series in lines:
            ...
            width = opt_num((series.get("lineStyle") or {}).get("width")
                            if isinstance(series.get("lineStyle"), dict)
                            else None, 2)
            data = series.get("data")
            data = data if isinstance(data, list) else []
            values = [opt_num(value, 0) for value in data[:count]]
            pts = [(plot["x"] + band * (index + 0.5), value_y(value, axis))
            ...
            if area is not None:
                canvas.save()
                canvas.alpha(1.0 if isinstance(area, dict)
                             and area.get("color") else 0.25)
    ...
    def draw_pie(self, canvas, chart):
        ...
        for index, entry in enumerate(data):
            ...
            slices.append((str(entry.get("name", index)),
        ...
        inner, outer = resolve(pair[0]), resolve(pair[1] if len(pair) > 1
                                                else "70%")
        item_style = series.get("itemStyle") \
            if isinstance(series.get("itemStyle"), dict) else {}
        ...
        if label is not False:
            formatter = label.get("formatter") if isinstance(label, dict) \
                and isinstance(label.get("formatter"), str) else "{b}"
        angle = -math.pi / 2
        for index, (name, value) in enumerate(slices):
            ...
            if formatter:
                ...
                text = (formatter.replace("{b}", name)
                        .replace("{c}", format_number(value))
                        .replace("{d}", format_number(round(share * 1000) / 10.0)))
                canvas.line_path(cx + math.cos(middle) * outer,
                                 cy + math.sin(middle) * outer, at_x, at_y)
                canvas.paint(stroke=color, width=1)
                self.chart_text(canvas, chart, at_x + (4 if right else -4),
                                at_y + 4, text,
                                parse_color(chart["labelColor"],
                                            (0.42, 0.45, 0.5, 1)), 12,
    ...
    def _pie_slice(self, canvas, cx, cy, inner, outer, start, end):
        def arc(radius, from_angle, to_angle, move):
            ...
            angle = from_angle
            if move:
                canvas.op("%s %s m" % (num(cx + math.cos(angle) * radius),
        ...
        if inner > 0:
            canvas.op("%s %s l" % (num(cx + math.cos(end) * inner),
                                   num(cy + math.sin(end) * inner)))
    ...
    def draw_legend(self, canvas, chart):
        ...
        if pie is not None:
            ...
            entries = [(str((entry or {}).get("name", index))
                        if isinstance(entry, dict) else str(index),
                        parse_color(chart["colors"][index % len(chart["colors"])]))
                       for index, entry in enumerate(data)]
        else:
            entries = [(str(series.get("name") or "Series %d" % (index + 1)),
                        self.series_color(chart, series, index))
                       for index, series in enumerate(chart["series"])]
        if not entries:
            return
        face = face_name(chart["font"], is_bold(legend["weight"]), False)
        widths = [legend["itemWidth"] + 8
        ...
        top = legend["top"] if legend["top"] is not None \
        ...
        for index, (name, color) in enumerate(entries):
            canvas.rect_path(left, swatch_y, legend["itemWidth"],
                             legend["itemHeight"],
                             min(3.0, legend["itemHeight"] / 2.0))
            canvas.paint(fill=color)
            self.chart_text(canvas, chart, left + legend["itemWidth"] + 8,
                            baseline, name,
                            parse_color(legend["color"], (0.42, 0.45, 0.5, 1)),
                            legend["size"], "start", legend["weight"])
            left += widths[index] + (legend["itemGap"]
