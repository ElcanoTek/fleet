...
| Nothing at all for a while | A model that reasons before it writes can be silent for a minute or more, and that is not a fault. By default the platform gives a model about a minute and a quarter to start, and longer in a long conversation, because a bigger prompt takes longer to read. If it still has not started, the platform tries again by itself, and your deployment may also try a backup model, so several minutes can pass before anything appears. Only then does a card say the model did not start responding. Retry from the card, or pick a different model. |
