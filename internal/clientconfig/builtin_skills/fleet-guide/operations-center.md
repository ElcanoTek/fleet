...
says why, and the failure email carries the same line.
...
the bad entry. A plain replay is refused, because it would rerun the same
broken line. An administrator can replay it with a corrected prompt
(`fleet sched dlq replay --prompt-file`), which resumes the schedule and keeps
its task memory. Editing the dead-lettered task does not fix the schedule:
saving an edit to a finished task starts a one-off run, not a new schedule.
Creating the task again with the corrected prompt and its schedule also works,
but the new schedule starts without the old one's task memory. Fleet refuses
to save such a line in the first place and names the bad entry, so this only
affects tasks saved before that check.
