...
var taskColumnRegistry = []taskColumn{
	...
	},
	{
		...
		read: true,
		// Why the breaker parked the chain (migration 072, ADR-0070/0073):
		// written with recurrence_parked_at, cleared with it.
		noInsert:   "runtime park reason (migration 072): written only with recurrence_parked_at by the breaker; a task insert must never set or clear it",
		noUpsert:   "runtime park reason (migration 072): a status write routed through the upsert must never clobber or stamp a parked chain's reason",
		noTxUpdate: "runtime park reason (migration 072): owned by the breaker and ReplayDeadLetteredTask, like recurrence_parked_at",
		noExport:   "runtime settlement (migration 072): a re-imported definition is not parked",
