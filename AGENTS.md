...
- **Conversation branching (#454)**: fork a chat conversation at a chosen message into a new independent thread.
  ...
  - **Deferred** (honest scope): the chat UI affordance (a "branch from here" control mapping the aggregated render-messages back to a DB message id, + navigation to the new conversation) is a follow-up; this PR ships the data model + API + the message-id exposure the UI needs to drive it. Lineage display ("forked from X") is likewise deferred.
