...
func TestClassifyInvocation(t *testing.T) {
	cases := []struct {
		...
		{"task run → taskrun (the folded cutlass harness)", []string{"task", "run", "x.yaml"}, invokeTaskRun},
		{"mcp test → in-binary probe", []string{"mcp", "test", "--all"}, invokeMCPTest},
		{"mcp reload → admin", []string{"mcp", "reload"}, invokeAdmin},
		{"bare mcp → admin (usage)", []string{"mcp"}, invokeAdmin},
		{"task export → admin", []string{"task", "export"}, invokeAdmin},
		{"bare task → admin (usage)", []string{"task"}, invokeAdmin},
