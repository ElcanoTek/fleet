...
// dispatch (not "unknown command"). We assert the dispatched code matches calling
...
func TestServiceVerbsWiredInDispatch(t *testing.T) {
	for _, verb := range []string{"restart", "stop", "logs", "tail"} {
		if code := dispatch([]string{verb, "--definitely-not-a-flag"}); code != 1 {
			t.Errorf("dispatch(%q --bad) = %d, want 1 (verb must be wired, parse-error exit 1)", verb, code)
		}
	}
}
