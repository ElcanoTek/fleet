...
Fleet embeds eight generally-useful skills in the binary
...
`fleet-guide` (fleet's own user guides, so the assistant can answer "how do I
use this?" from the same text `/help` renders —
[USER-GUIDES.md](USER-GUIDES.md)), `web-research-brief`,
`code-review-checklist`, `release-notes`, `executive-report`. Every bundle
inherits them by default — the skills analogue of the built-in MCP directory.
