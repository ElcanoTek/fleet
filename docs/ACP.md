...
| `session/prompt` | One `POST /chat` turn. Later prompts in the session continue the same fleet conversation. The conversation id is taken from the `X-Fleet-Conversation-Id` response header as well as the first frame, so a stream that dies early does not start a second conversation on retry. Each prompt carries an idempotency `input_id`: the client's `messageId` when it sends one (echoed back as `userMessageId`), otherwise a key that a retry of the same text reuses after a lost answer. fleet honours that key for prompts it **queued**, so a retry of a queued prompt is not run twice. It does not yet persist the key for a turn that started directly, so after a lost stream on a direct turn a retry can run the prompt again (see Honest scope). A prompt cancelled while fleet was queueing it is withdrawn from the queue. If the conversation already has a running turn (started from another surface, such as the web chat), fleet queues the prompt to run after it; the prompt then ends `end_turn` with a note saying it was queued and where to follow it, rather than an error inviting a retry. The response carries `_meta["fleet.conversationId"]` and token `usage`. |
...
- **Retry after a lost stream is safe only for queued prompts.** fleet's
  `input_id` idempotency covers queued inputs. A turn that started directly
  records no key, so if its stream is lost and the client resends the same
  prompt, fleet can run it a second time. Closing this needs fleet to persist
  the key for direct turns too (a server change, not an adapter one).
