...
`no-new-privileges`, a same-path workspace bind, and a per-turn network policy.
...
- New tool capabilities must work *through* the sandbox (e.g. via the
  `ExtraRunArgs` seam), not around it. This shapes how sandbox features such as
  a seccomp filter (#219) or an egress allowlist (#211) are added.
