"""
Tests for the validate_domains MCP tool for Media.net Select.

This tool is local-only (no HTTP calls) and validates domain format strings.
Tests should mirror OpenX validate_domains tests to ensure parity.
"""
...
from medianet_mcp import mn_validate_domains
...
class TestValidateDomains:
    ...
    async def test_valid_domains(self):
        ...
        result = await mn_validate_domains(domains)
    ...
    async def test_empty_list(self):
        ...
        result = await mn_validate_domains([])
    ...
    @pytest.mark.asyncio
    async def test_summary_all_invalid(self):
        """Test summary message when all domains are invalid."""
        domains = [
            "http://invalid.com",
            "https://also-invalid.com",
        ]
        ...
        assert len(result["valid"]) == 0
        assert len(result["invalid"]) == 2
        assert "All 2 domains are invalid" in result["summary"]

    @pytest.mark.asyncio
    async def test_summary_mixed(self):
        """Test summary message for mixed valid/invalid."""
        domains = [
            "valid.com",
            "http://invalid.com",
            "also-valid.org",
        ...
        assert "2 of 3 domains are valid" in result["summary"]
        assert "1 domains have issues" in result["summary"]

    @pytest.mark.asyncio
    async def test_no_http_calls_made(self, mock_medianet_api: respx.MockRouter):
        ...
        assert mock_medianet_api.calls.call_count == 0

    @pytest.mark.asyncio
    async def test_whitespace_in_domain(self):
        """Test that domains with internal whitespace are rejected."""
        domains = [
            "example .com",
            "exam ple.com",
        ]
        ...
        for invalid in result["invalid"]:
            assert "whitespace" in invalid["reason"].lower()

    @pytest.mark.asyncio
    async def test_various_tlds(self):
        """Test various valid TLDs are accepted."""
        domains = [
            "example.com",
            "example.org",
            "example.net",
    ...
    async def test_subdomains(self):
        """Test subdomains are correctly validated."""
        domains = [
            "www.example.com",
            "api.v2.example.com",
    ...
    @pytest.mark.asyncio
    async def test_hyphenated_domains(self):
        """Test hyphenated domains are correctly validated."""
        domains = [
            "my-site.com",
            "my-awesome-site.org",
            "a-b-c-d.co.uk",
