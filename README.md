...
same governed sandbox. The demos below tell one story: **plan the work in
chat, automate the follow-through, ride along from anywhere.** The web demos
are recordings of a real local stack — real model, real sandbox, real tools;
the TUI demo is a scripted recording against a mock server. Both pipelines are
documented in [`docs/generating-demo-gif.md`](docs/generating-demo-gif.md).
